...
begin
  ...
  loop
    ...
    v_remaining_for_invoice := coalesce((v_applied_detail->>'amount_applied')::numeric, 0);
    ...
    if v_target_line.id is not null then
      update public."SupplierInvoiceLine"
      set
        paid_amount = round((coalesce(paid_amount, 0)::numeric + v_remaining_for_invoice)::numeric, 2),
    ...
    for v_target_line in
      ...
      where supplier_id = p_supplier_id
        and invoice_number = v_applied_detail->>'invoice_number'
      order by ((coalesce(purchase_amount, 0)::numeric + coalesce(gst_amount, 0)::numeric) - coalesce(paid_amount, 0)::numeric) asc,
    ...
    loop
      ...
      update public."SupplierInvoiceLine"
      set
        paid_amount = round((coalesce(paid_amount, 0)::numeric + v_pay_amount)::numeric, 2),
        updated_date = v_updated_at
      where id = v_target_line.id;
