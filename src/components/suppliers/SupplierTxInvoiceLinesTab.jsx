                ...
                <ContextMenuTrigger asChild>
                  <TableRow className={`${line.isNew || line.id.startsWith('temp_') ? 'bg-blue-50' : index % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`} onClick={() => setSelectedLineId(line.id)}>
                    ...
                    <TableCell className={(typeof line.gst === 'number' && line.gst < 0) || (typeof line.gst === 'string' && line.gst !== '' && isNaN(parseFloat(line.gst))) ? 'text-red-600' : ''}>
                      <Input type="text" value={typeof line.gst === 'number' ? line.gst.toFixed(2) : line.gst || ''} onChange={(e) => !isDisabled && handleLineChange(line.id, 'gst', e.target.value)} onBlur={(e) => !isDisabled && handleValueBlur(line.id, 'gst', e.target.value)} onFocus={() => setSelectedLineId(line.id)} className={`text-right ${typeof line.gst === 'string' && line.gst !== '' && isNaN(parseFloat(line.gst)) ? 'text-red-600 border-red-300' : ''} ${isDisabled || !line.gst_override ? 'cursor-not-allowed' : ''}`} readOnly={isDisabled || !line.gst_override} />
                    </TableCell>
