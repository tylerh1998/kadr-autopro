  ...
  return (
    <div className="overflow-x-auto">
      <Table className="lines-table">
        <TableHeader>
          <TableRow>
            <TableHead className="w-[120px] cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700" onClick={() => requestSort('invoice_number')}>
            ...
            <TableHead className="w-[180px] cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700" onClick={() => requestSort('invoice_date')}>
            ...
            <TableHead className="cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700" onClick={() => requestSort('description')}>
            ...
            <TableHead className="w-[120px] text-right cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700" onClick={() => requestSort('charge')}>
            ...
            <TableHead className="w-[120px] text-right cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700" onClick={() => requestSort('gst')}>
            ...
            <TableHead className="w-[120px] text-right cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700" onClick={() => requestSort('line_total')}>
            ...
            <TableHead className="w-[200px] cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700" onClick={() => requestSort('gl_account')}>
        ...
        <tfoot className="print:table-footer-group hidden font-bold bg-gray-100 dark:bg-slate-800">
          <tr>
            <td colSpan={3} className="text-right p-1 border border-gray-300 dark:border-slate-600">Totals:</td>
            <td className="text-right p-1 border border-gray-300 dark:border-slate-600">${filteredInvoiceLines.reduce((sum, line) => sum + (parseFloat(line.charge) || 0), 0).toFixed(2)}</td>
            <td className="text-right p-1 border border-gray-300 dark:border-slate-600">${filteredInvoiceLines.reduce((sum, line) => sum + (parseFloat(line.gst) || 0), 0).toFixed(2)}</td>
            <td className="text-right p-1 border border-gray-300 dark:border-slate-600">${filteredInvoiceLines.reduce((sum, line) => sum + (parseFloat(line.line_total) || 0), 0).toFixed(2)}</td>
            <td colSpan={2} className="border border-gray-300 dark:border-slate-600"></td>
          </tr>
        </tfoot>
        <TableBody>
            ...
            return (
              <ContextMenu key={line.id}>
                <ContextMenuTrigger asChild>
                  <TableRow className={`${line.isNew || line.id.startsWith('temp_') ? 'bg-blue-50 dark:bg-blue-900/20' : index % 2 === 0 ? 'bg-white dark:bg-slate-900' : 'bg-slate-100 dark:bg-slate-800'}`} onClick={() => setSelectedLineId(line.id)}>
                    <TableCell>
                      <Input value={line.invoice_number || ''} onChange={(e) => !isDisabled && handleLineChange(line.id, 'invoice_number', e.target.value)} onFocus={() => setSelectedLineId(line.id)} readOnly={isDisabled || line.inventory} className={`bg-white dark:bg-slate-950 dark:border-slate-600 dark:text-slate-100 ${isDisabled || line.inventory ? 'cursor-not-allowed' : ''}`} />
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-1">
                        <Input type="text" value={line.invoice_date && line.invoice_date.length === 10 && line.invoice_date.includes('-') ? formatDateForInput(line.invoice_date) : line.invoice_date || ''} onChange={(e) => !isDisabled && handleLineChange(line.id, 'invoice_date', e.target.value)} onBlur={(e) => !isDisabled && handleDateBlur(line.id, e.target.value)} onFocus={() => setSelectedLineId(line.id)} placeholder="MM/DD/YYYY" className={`w-[6.25rem] bg-white dark:bg-slate-950 dark:border-slate-600 dark:text-slate-100 ${line.dateError ? 'text-red-600 border-red-500' : ''} ${isDisabled || line.inventory ? 'cursor-not-allowed' : ''}`} readOnly={isDisabled || line.inventory} title={line.dateError || ''} />
                        <Popover>
                    ...
                    <TableCell>
                      <Textarea value={line.description || ''} onChange={(e) => !isDisabled && handleLineChange(line.id, 'description', e.target.value)} onFocus={() => setSelectedLineId(line.id)} rows={2} className={`resize-none bg-white dark:bg-slate-950 dark:border-slate-600 dark:text-slate-100 ${isDisabled || line.inventory ? 'cursor-not-allowed' : ''}`} readOnly={isDisabled || line.inventory} />
                    </TableCell>
                    <TableCell className={(typeof line.charge === 'number' && line.charge < 0) || (typeof line.charge === 'string' && line.charge !== '' && isNaN(parseFloat(line.charge))) ? 'text-red-600' : ''}>
                      <Input type="text" value={typeof line.charge === 'number' ? line.charge.toFixed(2) : line.charge || ''} onChange={(e) => !isDisabled && handleLineChange(line.id, 'charge', e.target.value)} onBlur={(e) => !isDisabled && handleValueBlur(line.id, 'charge', e.target.value)} onFocus={() => setSelectedLineId(line.id)} className={`text-right bg-white dark:bg-slate-950 dark:border-slate-600 dark:text-slate-100 ${typeof line.charge === 'string' && line.charge !== '' && isNaN(parseFloat(line.charge)) ? 'text-red-600 border-red-300' : ''} ${isDisabled || line.inventory ? 'cursor-not-allowed' : ''}`} readOnly={isDisabled || line.inventory} />
                    </TableCell>
                    <TableCell className={(typeof line.gst === 'number' && line.gst < 0) || (typeof line.gst === 'string' && line.gst !== '' && isNaN(parseFloat(line.gst))) ? 'text-red-600' : ''}>
                      <Input type="text" value={typeof line.gst === 'number' ? line.gst.toFixed(2) : line.gst || ''} onChange={(e) => !isLockedByOtherUser && lockAcquired && !locked && line.gst_override && handleLineChange(line.id, 'gst', e.target.value)} onBlur={(e) => !isLockedByOtherUser && lockAcquired && !locked && line.gst_override && handleValueBlur(line.id, 'gst', e.target.value)} onFocus={() => setSelectedLineId(line.id)} className={`text-right bg-white dark:bg-slate-950 dark:border-slate-600 dark:text-slate-100 ${typeof line.gst === 'string' && line.gst !== '' && isNaN(parseFloat(line.gst)) ? 'text-red-600 border-red-300' : ''} ${isLockedByOtherUser || !lockAcquired || locked || !line.gst_override ? 'cursor-not-allowed' : ''}`} readOnly={isLockedByOtherUser || !lockAcquired || locked || !line.gst_override} />
                    </TableCell>
                    <TableCell className={(typeof line.line_total === 'number' && line.line_total < 0) || line.line_total === 'Error' ? 'text-red-600' : ''}>
                      <Input type="text" value={typeof line.line_total === 'number' ? line.line_total.toFixed(2) : line.line_total || ''} onChange={(e) => !isDisabled && handleLineChange(line.id, 'line_total', e.target.value)} onBlur={(e) => !isDisabled && handleValueBlur(line.id, 'line_total', e.target.value)} onFocus={() => setSelectedLineId(line.id)} className={`text-right bg-white dark:bg-slate-950 dark:border-slate-600 dark:text-slate-100 ${line.line_total === 'Error' ? 'text-red-600 border-red-300' : ''} ${isDisabled || line.inventory ? 'cursor-not-allowed' : ''}`} readOnly={isDisabled || line.inventory} />
                    </TableCell>
                    <TableCell>
                      {(() => {
