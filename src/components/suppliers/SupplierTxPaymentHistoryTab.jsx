    ...
    <Card>
      ...
      <CardContent>
        {loading ? (
          ...
          <div className="text-center py-12"><FileText className="w-12 h-12 mx-auto text-slate-400 mb-4" /><h3 className="text-lg font-semibold text-slate-900 mb-2">No Payments</h3><p className="text-slate-600">No payment history found for this supplier.</p></div>
        ) : (
                  ...
                  return (
                    <React.Fragment key={payment.id}>
                      <TableRow className={`hover:bg-slate-100 cursor-pointer ${payments.indexOf(payment) % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`} onClick={() => togglePaymentExpansion(payment.id)}>
                        <TableCell className="w-8">{appliedInvoices.length > 0 && (isExpanded ? <ChevronDown className="w-4 h-4 text-slate-500" /> : <ChevronRight className="w-4 h-4 text-slate-500" />)}</TableCell>
                          ...
                          <TableCell colSpan={7} className="bg-slate-50 p-0">
                            <div className="p-4 pl-12">
                              ...
                              <div className="bg-white rounded border">
                                <Table>
                                  <TableHeader>
                                    <TableRow className="bg-slate-100"><TableHead className="text-xs">Invoice Number</TableHead><TableHead className="text-xs text-right">Amount Applied</TableHead></TableRow>
                                  </TableHeader>
                                  <TableBody>
                      ...
                      {isExpanded && appliedInvoices.length === 0 && payment.invoice_number === 'On Account' && (
                        <TableRow><TableCell colSpan={7} className="bg-slate-50 p-0"><div className="p-4 pl-12 text-sm text-slate-600">This payment was applied "On Account" and not allocated to specific invoices.</div></TableCell></TableRow>
                      )}
                    </React.Fragment>
                  );
                })}
