  ...
  const renderVehicleCard = (vehicle, type) => (
    <div className={`p-4 rounded-lg border ${type === 'master' ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
      ...
      <div className="space-y-1">
        <div className="font-bold text-lg">{getVehicleName(vehicle)}</div>
        <div className="text-sm text-gray-600">VIN: {vehicle.vin || 'N/A'}</div>
        <div className="text-sm text-gray-600">Plate: {vehicle.license_plate || 'N/A'}</div>
        <div className="text-xs text-gray-400 mt-2">ID: {vehicle.id}</div>
        ...
        <div className="mt-2 pt-2 border-t border-gray-200/50 grid grid-cols-2 gap-2 text-xs">
          ...
          <div>
            <span className="text-gray-500">Odo Date:</span>
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle>Merge Vehicles</DialogTitle>
        </DialogHeader>
        ...
        {step === 1 ? (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 p-3 rounded text-sm grid grid-cols-2 gap-4">
                <div>
                    <span className="text-gray-500 block">New Mileage</span>
                    <span className="font-medium">
                        {Math.max(parseFloat(masterVehicle.mileage || 0), parseFloat(duplicateVehicle.mileage || 0))}
                    </span>
                </div>
                <div>
                    <span className="text-gray-500 block">New Odometer Date</span>
                    <span className="font-medium">
                         {
