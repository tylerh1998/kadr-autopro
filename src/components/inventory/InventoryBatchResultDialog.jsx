        ...
        <div className="space-y-4">
          ...
          {successCount > 0 && (
            <div className="rounded-lg border border-green-200 bg-green-50 p-4">
              <div className="mb-3 flex items-center gap-2 text-green-800 font-semibold">
                <CheckCircle2 className="w-4 h-4" />
                Successful Invoices
              </div>
              <div className="space-y-2">
                {successfulInvoices.map((result, index) => (
                  <div key={`${result.invoice}-${index}`} className="text-sm text-green-900">
                    • {result.invoice}
                  </div>
                ))}
              </div>
          ...
          {failedCount > 0 && (
            <div className="rounded-lg border border-red-200 bg-red-50 p-4">
              <div className="mb-3 flex items-center gap-2 text-red-800 font-semibold">
                <XCircle className="w-4 h-4" />
                Failed Invoices
              </div>
              <div className="space-y-3">
                {failedInvoices.map((result, index) => (
                  <div key={`${result.invoice}-${index}`} className="rounded-md bg-white/70 p-3 text-sm border border-red-100">
                    <div className="font-medium text-red-900">{result.invoice}</div>
                    <div className="mt-1 whitespace-pre-wrap text-red-700">{result.message}</div>
                  </div>
                ))}
              </div>
            </div>
