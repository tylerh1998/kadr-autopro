...
export default function NoteColumn({ cards, onSelect, onColourChange }) {
  return (
    <div className="min-h-[520px] space-y-3">
      {cards.length > 0 ? (
        cards.map((card) => <NoteCard key={card.id} card={card} onSelect={onSelect} onColourChange={onColourChange} />)
      ) : (
