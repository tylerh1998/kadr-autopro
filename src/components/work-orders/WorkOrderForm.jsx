...
function padLines(lines, minLines = 20, defaultTaxable = true) { // Added defaultTaxable parameter
  const blankLine = {
    ...
    labour: '',
    total: 0,
    taxable: defaultTaxable, // Explicitly set taxable
    complete: false,
    bold: false,
    ...
    qty_on_order: 0,
    unit: '',
    manually_inserted: false // Default to false for automatically padded lines
  };
  
  const paddedLines = [...lines];
  while (paddedLines.length < minLines) {
    // Create a new unique ID for each blank line
    paddedLines.push({ 
      ...blankLine, 
      id: `blank_${Date.now()}_${Math.random()}_${paddedLines.length}` 
    });
  }
...
export default function WorkOrderForm({
  ...
  const tracedSetLineItems = useCallback((updater) => {
    ...
    setDisplayLineItems(prevDisplayLines => {
      ...
      // Include manually inserted lines even if they're blank
      const actualLines = updatedDisplayLines.filter(line => 
        line && (line.description || line.part_number || line.inventory_item_id || line.is_other_charge || line.manually_inserted)
      );
      
      console.log('WorkOrderForm: Updating parent with actual lines:', actualLines.length);
      ...
      return padLines(actualLines, 20, defaultTaxable); // Return the padded version for local state
    });
  }, [setParentLineItems]);
    ...
    tracedSetLineItems(prev => {
        ...
        partsArrayWithProcessedFlags.forEach(newLineItem => {
            ...
            if (currentUpdateIndex !== -1 && updated[currentUpdateIndex] && (!updated[currentUpdateIndex]?.description && !updated[currentUpdateIndex]?.part_number && !updated[currentUpdateIndex]?.manually_inserted)) {
              ...
              if (nextEmptyIndex !== -1) {
                  updated[nextEmptyIndex] = { ...lineWithTaxable, id: Date.now() + Math.random() };
            ...
            if (!emptyLineFound) {
                updated.push({ ...lineWithTaxable, id: Date.now() + Math.random() });
  ...
  const handleReceiveWorkOrderPart = async (lineItem, receivedQuantity) => {
      ...
      // The backend function has already handled all the database updates
      // We just need to update the local state to reflect the changes
      tracedSetLineItems(prev => {
    ...
    const handleKeyDown = (e) => {
      ...
      switch(e.key.toLowerCase()) {
        ...
        case 'x':
          ...
          tracedSetLineItems(prev => {
            const updated = [...prev];
            if (updated[selectedLineIndex]) {
              updated[selectedLineIndex] = {
                ...updated[selectedLineIndex],
                taxable: !updated[selectedLineIndex].taxable
              };
            }
            return updated;
          });
