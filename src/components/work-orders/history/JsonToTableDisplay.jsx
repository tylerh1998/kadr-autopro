...
export default function JsonToTableDisplay({ data, compareData }) {
  ...
  const changedFieldKeys = useMemo(() => {
    return Array.from(new Set([...Object.keys(previousData), ...Object.keys(currentData)]))
      ...
      .filter((key) => !valuesEqual(previousData[key], currentData[key]))
      .sort();
        ...
        <div className="space-y-2">
          ...
          <Card>
            <CardContent className="p-3 flex flex-wrap justify-around items-center gap-x-4 gap-y-3">
              ...
              {changedFinancialKeys.includes('labor_total') && <HistoryFinancialItem label="Labour Total" currentValue={currentData.labor_total} previousValue={previousData.labor_total} />}
              {changedFinancialKeys.includes('shop_supply_total') && <HistoryFinancialItem label="Shop Supplies" currentValue={currentData.shop_supply_total} previousValue={previousData.shop_supply_total} />}
              {changedFinancialKeys.includes('tax_amount') && <HistoryFinancialItem label="GST (5%)" currentValue={currentData.tax_amount} previousValue={previousData.tax_amount} />}
              {changedFinancialKeys.includes('total_amount') && <HistoryFinancialItem label="Total" currentValue={currentData.total_amount} previousValue={previousData.total_amount} />}
              {changedFinancialKeys.includes('amount_paid') && <HistoryFinancialItem label="Payments" currentValue={currentData.amount_paid} previousValue={previousData.amount_paid} />}
            </CardContent>
          </Card>
        </div>
      )}
