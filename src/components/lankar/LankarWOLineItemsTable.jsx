      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-slate-100 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800">
            <TableHead className="w-20 text-center font-bold text-slate-700 dark:text-slate-300">Qty</TableHead>
            <TableHead className="w-20 text-center font-bold text-slate-700 dark:text-slate-300">Hrs</TableHead>
            <TableHead className="font-bold text-slate-700 dark:text-slate-300">Description</TableHead>
            <TableHead className="w-28 text-right font-bold text-slate-700 dark:text-slate-300">Unit Price</TableHead>
            <TableHead className="w-24 text-right font-bold text-slate-700 dark:text-slate-300">Parts</TableHead>
            <TableHead className="w-24 text-right font-bold text-slate-700 dark:text-slate-300">Labor</TableHead>
            <TableHead className="w-24 text-right font-bold text-slate-700 dark:text-slate-300">Other</TableHead>
            <TableHead className="w-12 text-center font-bold text-slate-700 dark:text-slate-300">Tax</TableHead>
            <TableHead className="w-24 text-right font-bold text-slate-700 dark:text-slate-300">Total</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
              ...
              <TableRow key={line.id || index} className={`${rowBgClass} hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors`}>
                ...
                <TableCell className="min-w-0 flex-1 pr-2 p-2 align-top">
                  <div className="space-y-1">
                    ...
                    {line.partnum && (
                      <p className="text-xs text-slate-500 dark:text-slate-400"><span className="font-medium text-slate-600 dark:text-slate-400">Part #:</span> {line.partnum}</p>
                    )}
                    {line.c !== null && line.c !== undefined && String(line.c).trim() !== '' && Number(line.c) !== 0 && (
                      <p className="text-xs text-slate-500 dark:text-slate-400"><span className="font-medium text-slate-600 dark:text-slate-400">Cores:</span> {line.c}</p>
                    )}
                    {line.partserial && (
                      <p className="text-xs text-slate-500 font-mono"><span className="font-medium text-slate-600 dark:text-slate-400 not-italic font-sans">Serial #:</span> {line.partserial}</p>
                    )}
                  </div>
                </TableCell>
                ...
                <TableCell className="w-12 p-2 align-top text-center">
                  {taxable ? <CheckCircle className="w-4 h-4 text-green-600 dark:text-green-400 mx-auto" /> : <XCircle className="w-4 h-4 text-red-600 dark:text-red-400 mx-auto" />}
                </TableCell>
