...
Deno.serve(async (req) => {
  try {
    ...
    switch (action) {
      case 'list':
        result = await supabase.from('Customer').select('*').order('org_name', { ascending: true });
        break;
      case 'filter':
      ...
      case 'get':
        ...
        break;
      case 'create':
        result = await supabase.from('Customer').insert(data).select().single();
        break;
      case 'update':
        result = await supabase.from('Customer').update(data).eq('id', id).select().single();
        break;
      case 'delete':
        result = await supabase.from('Customer').delete().eq('id', id);
        break;
      default:
