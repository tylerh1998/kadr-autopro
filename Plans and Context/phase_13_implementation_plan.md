...
**Status:** IN PROGRESS — 13A code-complete, deployed to dev branch, awaiting manual UI verification before production RPC deploy and starting 13B
...
- [ ] `useWorkOrder.jsx`'s native fetch tested: open a real WO in `DocumentEditor.jsx`, confirm customer/vehicle/line-item data loads correctly
- [ ] JSON-column audit complete — grep for `JSON.parse(workOrder.`/`JSON.parse(wo.` across all 3 files, defensive guards applied, tested with a real WO with non-empty `line_items`/`payments`/`accounting_details`/`tech_time`
- [ ] `InvoiceConversion.jsx`'s WorkOrder fetch converted and tested
- [ ] `search_work_orders` RPC deployed to dev branch; output compared row-for-row against the old JS `getworkorderlist` for: default list, invoice-tab filter, every sort preset, and a real search term (RO number, customer name)
- [ ] Same RPC applied to production after dev verification
- [ ] "New Work Order" and "Counter Sale" creation both tested; RO-number counter confirmed incrementing correctly
- [ ] `autopro-changeWorkOrderCustomer` tested with a WO that has existing linked payments
- [ ] `SystemSettings`/`WorkOrderStatus` reads converted and tested in all 4 files
- [ ] `npm run build` clean
