...
**Status:** **In Progress.** 10A, 10B, 10C, and 10D are all **done and confirmed live** on `test.kensauto.ca` (10A via commit `2b21dccaf`; 10B via `edd03df7` + `fdaff904` + `8429b218`/`9e6c1d3b`; 10C frontend via commit `46c86e2b` "Phase 10C", backend functions deployed directly to both branches ahead of that push; 10D pushed and live-verified 2026-08-03, including a real bug found and fixed — `CashFlowSummary`'s 12 `bigint` fields silently rejected cents, widened to `double precision` on both branches). See "Phase Results and Final Context" below for the full rollup of all four. **10E (Levies) code is complete and backend-verified as of 2026-08-03** — schema work done (`Levies.total_amount`/`base_amount` widened to `double precision` on dev, `Levies` table + `OtherChargeList` data replayed fresh to production), all 3 functions (`autopro-syncLevies`, `autopro-getReportableLeviesReport`, `autopro-postLeviesToAP`) ported with the `checkFiscalPeriodStatus()` guard on the posting function, deployed to both dev and production, curl/SQL-verified end-to-end on dev with throwaway data (delta-ledger reconciliation, report totals matching a manual SQL sum to the dollar, balanced GL entries). Both frontend call sites converted, repo-wide grep clean, `npx vite build` clean. **Not yet pushed** — live-UI verification on `test.kensauto.ca` is the only remaining item, same as every prior sub-phase's final gate.
...
| **10E** | Levies: new schema, 3 native functions, 2 frontend call sites | Code complete, backend-verified on dev, deployed to production — live-UI pass pending push | **10A**-adjacent (own schema, low interdependency) |
...
> **Update (2026-08-03): code complete, backend fully verified on dev, deployed to production.** All schema work, all 3 functions, and both frontend call sites are done. Live-UI verification (real WO save trigger, real `/dev-login` session on the report page) is the only remaining item — same "not yet pushed" gate every prior sub-phase held at this point.
...
- [x] Widened `Levies.total_amount`/`base_amount` to `double precision` on dev — confirmed via `information_schema.columns`, all 153 real rows survived (`count(*)` unchanged before/after). `Levies` replayed to production as a fresh `CREATE TABLE` (table didn't exist there at all) with the corrected types baked in + the standard RLS policy (confirmed 1 policy via `pg_policies`). `OtherChargeList` seeded on production with dev's real 42 rows (not 40 as originally estimated — fetched fresh via a generated `INSERT` built from dev's live table, confirmed `count(*) = 42` on production post-insert). Migration recorded at `supabase/migrations/20260809000000_levies_table_and_bigint_fix.sql`.
- [x] `autopro-syncLevies` ported, deployed to dev, curl-verified with a throwaway work order (`test10ethrowaway0001`) exercising all 3 reconciliation paths in sequence: add (qty 4 → delta +4/+$20), modify (qty 4→2 → delta -2/-$10), delete (line removed → delta -2/-$10) — net ledger confirmed via SQL to be exactly 0/0 after all 3 calls, matching the legacy's append-only delta-ledger design exactly (not a naive create/overwrite). Test rows deleted after, dev's real 153 rows unaffected throughout.
- [x] `autopro-getReportableLeviesReport` ported, deployed to dev, curl-verified against the real 153 existing `Levies` rows for the full `2026-01-01`–`2026-07-31` range — returned all 153 rows, `sum(total_amount)` from the function response matched a manual SQL `sum(total_amount)` exactly ($1,286). `ro_number` correctly resolves to `'Unknown'` for all of them, which is expected (not a bug) — dev only has 3 real `WorkOrder` rows total, so none of the real historical levies' `work_order_id`s resolve on dev; the resolution logic itself (direct match + per-field fallback scan) is unchanged from legacy.
- [x] `autopro-postLeviesToAP` ported, deployed to dev, curl-verified via a synthetic throwaway period: inserted one throwaway `Levies` row dated in the open Oct–Dec 2026 fiscal period (avoided real data's Jan–Jul 2026 date range entirely, so no real levies were touched), posted it — confirmed balanced GL entries (debit 5001 $20 = credit 2000 $20), correct invoice numbering (`Q4-2026`, `invoice_date` = actual quarter-end `2026-12-31`), and the `Levies.supplier_invoice_line_id` batch-update landed correctly. Also verified the two guard paths: an empty-period request correctly returned `'No levies found for this period'` with zero writes, and a request dated into the closed Jan–Mar 2026 period correctly returned `'Fiscal period closed'` with zero writes (confirmed via SQL — no stray `SupplierInvoiceLine`/`GLTransaction` rows). Owned the dangling-supplier-id caveat from 10E.0 rather than creating a disposable `OtherChargeList` row — didn't matter for the test, the insert succeeds regardless since there's no FK constraint. All test rows (`Levies`, `SupplierInvoiceLine`, `GLTransaction`) deleted afterward; dev's real `Levies` count confirmed back to exactly 153.
- [x] `useDocumentEditorSave.jsx` and `ReportableLeviesReport.jsx` converted per 10E.3 — the response-shape unwrapping change applied correctly in both `ReportableLeviesReport.jsx` call sites (`supabase.functions.invoke` returns `{ data, error }` with `data` already being the function's JSON body, one level shallower than the legacy `base44.functions.invoke`'s nested `response.data.data` shape). `base44` imports dropped from both files (confirmed via grep, neither had any other `base44` usage).
- [ ] A real WO save triggers `autopro-syncLevies` correctly (live, via `/dev-login`); `ReportableLeviesReport.jsx` shows real data and posts to AP correctly through the actual UI (throwaway data, cleaned up after). **Not yet done — needs push first.**
- [x] Repo-wide grep clean of `syncLevies`/`getReportableLeviesReport`/`postLeviesToAP` base44 invokes (zero remaining — only the 3 new native `supabase.functions.invoke('autopro-*')` call sites found); `npx vite build` clean (fresh `dist/` output confirmed, zero errors).
- [x] Applied all 3 functions to production (`verify_jwt: false`, matching this phase's pattern) after dev verification passed — `OtherChargeList` production seed was done first (in the schema step above), same "seed before deploy" ordering as 10C.0's `SystemSettings` lesson. Not curl-tested directly against production (standing caution from 10C/10D — production functions get their first exercise via live UI, not curl); the schema-level seed (`Levies` table + `OtherChargeList` 42 rows) was verified via direct SQL instead.

**Resume point:** push the working tree (3 new edge functions already deployed to both dev and production ahead of push, 2 modified frontend files + 1 new migration file in the working tree), then run the live-UI verification checklist below on `test.kensauto.ca`.

### 10E.5) Verification Checklist

- [x] `autopro-syncLevies`: verified via curl + direct SQL (see 10E.4) — a throwaway line saved with different quantities across 3 calls produced the correct net delta in `Levies` (3 separate delta rows: +4/-2/-2, net 0/0), not a duplicate full record.
- [x] `autopro-getReportableLeviesReport`: report data matches a manual SQL cross-check for the same date range (153 rows, `sum(total_amount)` = $1,286 both ways).
- [x] `autopro-postLeviesToAP`: GL entries balance (debit 5001 = credit 2000), invoice number/date matched the expected quarter format (`Q4-2026` / `2026-12-31`), the posted `Levies` row's `supplier_invoice_line_id` updated correctly.
- [ ] Live UI: `ReportableLeviesReport.jsx`'s Run Report / Post to AP / Print flow all work through a real logged-in session on `test.kensauto.ca`. **Still open — needs push.**
- [ ] Live UI: a real work order save (via `/dev-login`) with a levy-bearing line item correctly triggers the sync, confirmed via reload. **Still open — needs push.**
- [x] Repo-wide grep clean; `npx vite build` clean.
- [x] Applied to production after dev verification (see 10E.4).

**🛑 HOLD FOR TESTING — Phase 10 is not complete until you've confirmed all applicable sub-phases above.** 10E's backend (schema + all 3 functions) is fully verified on dev and deployed to both branches; only the live-UI pass on `test.kensauto.ca` remains, pending your push.
...
**10E — research complete, both open questions resolved, code not started.** Full fresh research pass done 2026-08-03: all 3 legacy function sources (`syncLevies`/`getReportableLeviesReport`/`postLeviesToAP`) and both frontend call sites read in full, real schema/data state confirmed on both branches via direct SQL. `Levies` doesn't exist on production yet (genuinely fresh `CREATE TABLE` needed); `OtherChargeList` exists there but is empty (needs its real 40 rows seeded, same class of gap as 10C.0's `SystemSettings` finding). **Confirmed 2026-08-03:** widen `Levies.total_amount`/`base_amount` to `double precision` preemptively, and add the `checkFiscalPeriodStatus()` guard to `autopro-postLeviesToAP` — both baked into the 10E section above, no longer open questions.
...
2. Work 10E's task list top to bottom: schema first (widen `Levies` bigint columns on dev, then fresh-create on production with corrected types + `OtherChargeList` production seed), then port+deploy+curl-verify all 3 functions on dev (with the fiscal-period guard baked into `autopro-postLeviesToAP` from the start), then convert both frontend files (watch the response-shape unwrapping change specifically), then deploy to production, then run 10E.5's live verification checklist.
