...
**Status:** **In Progress.** 10A and 10B are both **done and confirmed live** on `test.kensauto.ca` (10A via commit `2b21dccaf`; 10B via `edd03df7` + `fdaff904` + `8429b218`/`9e6c1d3b`, confirmed via `git log` 2026-08-03; 10B.5's full verification checklist confirmed by you directly in the plan). **10C (GST & Taxes) is next — research for it is complete as of 2026-08-03 (see 10C below for full detail); code has not yet been written.** A critical pre-flight finding surfaced during 10C research: **production `SystemSettings`/`CashFlowSummary`/`OtherChargeList` all have zero rows** — 10A's "production replay" only replayed empty table schemas, not the actual data rows. 10C's task list includes the exact seed SQL needed before any GST function can work on production.
...
| **10C** | GST & Taxes: `GSTReturn` CRUD, 3 native functions, `Taxes.jsx`, `MarkPaidModal.jsx` (both fixes) | **Research complete, code not started** | **10A** (needs `GSTReturn`/`SystemSettings` schema — done); benefits from 10B's patterns |
...
> **Research complete (2026-08-03) — everything below is confirmed via direct source reads and live SQL against both branches, not assumption. Code has not been written yet.** This section is self-contained; no other file needs to be re-read to start executing.
...
- [ ] **Seed production `SystemSettings`** with dev's real row (10C.0) — do this first, before deploying any function to production, or `calculateGSTReturn`/`postGSTJournalEntries`/`processGSTPayment` will all fail there with `'System settings not found'`.
- [ ] `autopro-calculateGSTReturn` ported, deployed to dev, curl-verified against a real dev period (e.g. `2026-04-01`/`2026-06-30` — cross-check the result against the existing `GSTReturn` row for that exact period: `gst_collected 11570.12`, `gst_paid 6959.51`, `net_gst_due 4610.61` — this is a strong, free cross-validation since the historical data is already known-correct).
- [ ] `autopro-postGSTJournalEntries` ported, deployed to dev, curl-verified with a throwaway posting (Jul-Sep 2026 is unposted and safe — see 10C.1), confirm the resulting `GLTransaction` rows balance and match the sign-aware logic, then decide whether to leave it posted (it's real, uncontested data) or clean it up.
- [ ] `autopro-processGSTPayment` ported, deployed to dev, curl-verified (mark the same throwaway Jul-Sep return as paid, confirm `BankTransaction` + `GLTransaction` + `GSTReturn.status` all update, confirm `autopro-calculateBankBalances` fires and `BankAccount.current_balance` updates).
- [ ] `Taxes.jsx` and `MarkPaidModal.jsx` converted per 10C.4; `base44` imports dropped from both.
- [ ] Repo-wide grep clean of `calculateGSTReturn`/`postGSTJournalEntries`/`processGSTPayment`/`SupabaseProxy` (the `MarkPaidModal.jsx` usage) and `base44.entities.GSTReturn`; `npx vite build` clean.
- [ ] Apply all 3 functions to production after dev verification (production `SystemSettings` seed from step 1 must be done first).

### 10C.6) Verification Checklist

- [ ] `Taxes.jsx`: calculate a GST return for a real period, figures match a manually-verified calculation against `GLTransaction` (the Apr-Jun 2026 cross-check from the task list above satisfies this).
- [ ] Post a throwaway GST return (Jul-Sep 2026) → journal entries → mark paid via `MarkPaidModal.jsx` → confirm `BankTransaction`/GL/`GSTReturn.status` all update correctly. Since Jul-Sep 2026 is real (uncontested) data rather than disposable test data, decide with the user whether to leave it posted/paid for real or reverse it — don't unilaterally delete a real GST filing without asking.
- [ ] Confirm the bigint-vs-text account-number comparison (10C.2) actually works against real data — non-zero `gst_collected`/`gst_paid` in the calculate step is the tell.
...
**10C — research complete, code not started.** Full research pass done this session (2026-08-03): both legacy function sources read in full, `Taxes.jsx`/`MarkPaidModal.jsx` (taxes, not payroll) read in full, real data state confirmed on both branches via direct SQL. Everything needed to execute is now written into the 10C section above — **read 10C.0 through 10C.6 in full before starting**, don't re-derive from scratch. The single most important thing to act on first: **10C.0's finding that production `SystemSettings` has zero rows** (10A's "production replay" only replayed schema, not data) — seed it before deploying any 10C function to production, using the exact approach described in 10C.0/10C.5.

### Exact resume steps for the next session

1. Re-read 10C.0 through 10C.6 above in full (it's self-contained — legacy logic, real account numbers, real `GSTReturn`/`BankAccount` data, the bigint-vs-text casting note, and the exact frontend conversion table are all there).
2. Work 10C's task list top to bottom: seed production `SystemSettings` first, then port+deploy+curl-verify the 3 functions on dev, then convert `Taxes.jsx`/`MarkPaidModal.jsx`, then deploy to production, then run 10C.6's live verification checklist.
...
4. Once 10C.6 is fully confirmed by the user, proceed to 10D (Cash Flow & Bank Transfer) per the sub-phase order above — note 10D will hit the same "`CashFlowSummary` empty on production" gap flagged in 10C.0, worth a fresh direct-SQL check before assuming it's been fixed in the meantime.
