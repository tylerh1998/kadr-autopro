...
### Live verification session (2026-08-03, via `/dev-login` on `test.kensauto.ca`)

You confirmed the dev-branch `/dev-login` mechanism (from the Phase 3 work) is usable again — signed in as `test@kensauto.ca` against the dev Supabase branch. Findings:

- **`/Schedule` loads cleanly, zero console errors from the `Appointment`/`Employee`/`Customer`/`Vehicle` calls.** Calendar renders empty as expected (0 rows pre-go-live).
- **Confirmed pre-existing, out-of-scope structural limitation (not caused by this phase):** `Schedule.jsx`'s `loadData` bundles the still-base44-routed `getworkorderlist()` call into the same `Promise.all` as the migrated `Appointment`/`Employee`/`Customer`/`Vehicle` calls. On the dev-branch session, `getworkorderlist` 401s (base44-proxy doesn't recognize a dev-native Supabase JWT — the same Auth-isolation finding from Phase 3/master_context.md), which fails the whole `Promise.all` and silently discards the otherwise-successful direct-`supabase.from()` results. This blocks the customer/vehicle dropdowns in the New Appointment form and blocks `WorkOrders.jsx`'s WO list (same pattern) — which in turn blocks reaching `SchedulerViaWoModal`/`EditApptViaWoModal` through the UI at all on this dev session. Confirms why Section 4's WO-context items are correctly scoped as "deferred to Phase 13" — not a Phase 12 regression, just a pre-existing coupling this phase didn't touch (`WorkOrder` migration is Phase 13's job).
