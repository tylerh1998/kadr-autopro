...
- [x] All 7 frontend files edited per Section 3.1–3.7 (plus 0.6 `ReportModal.jsx` bug fix); build verified clean (`dist/` rebuilt, no compile errors)
- [x] All 6 Edge Functions written as verbatim ports (Section 3.1–3.6) and deployed to **both** production (`hbcrwkmgsazqrvsrmxyr`) and dev branch (`sitihbdnuxifwibontcm`)
- [x] **[Agent]** Auth-gating verified live for all 6 functions: no-Authorization-header request → gateway-level `401` (expected — `verify_jwt: true` at the Supabase Functions gateway, not app-level); a syntactically-valid-but-non-user bearer token (anon key) → app-level `{"error":"Unauthorized"}` at `200`, confirming the always-`200`/error-in-body convention (Section 0.7) end-to-end, including `autopro-getRealTimeInventoryOnOrder`'s new auth check (Section 3.6 note — previously had zero auth gating)
- [x] **[Agent]** Repo-wide grep for all 6 old function names + `SupabaseProxy`+`get_parts_movement_v2` returns zero hits in `src/` (only remaining `base44` reference in `src/components/reports/` is the out-of-scope `ReportableLeviesReport.jsx`, expected per Section 0.4)
- [x] **[Agent]** `get_parts_movement_v2` reconfirmed live as `SECURITY INVOKER`, matching Section 3.7's claim (no RLS regression risk from calling it directly)
- [~] **[Agent]** Full authenticated baseline JSON diff (base44 vs. native, same date range) **not performed** — this phase's agent session had no valid end-user session token to drive `supabase.auth.getUser()` past the app-level auth gate (dev-login credentials weren't available in this session), so true side-by-side output comparison wasn't possible from here. Substituted: (1) every function is a verbatim logic port with only the documented entity→table swaps (Section 3.1–3.6), (2) SQL sanity check against production confirmed real data exists in the query ranges each function targets (172 invoiced WOs, 77 open WOs, 63 estimates in a representative July 2026 range; 1,461 customers; 4,441 inventory items — all match the row-count context cited elsewhere in this plan), (3) build is clean and all call sites are wired correctly. **Recommend the [User] click-through step below double as the missing baseline check** — first run of each report post-repoint is effectively the real-data verification this item couldn't complete.
- [x] **[Agent]** Confirmed zero file overlap with the concurrently-running Phase 5 — this phase only touched `src/components/reports/*.jsx` + new `supabase/functions/autopro-get*` dirs, neither of which appear in Phase 5's file list (Section 0.2)
