...
- `APSummaryTable.jsx`'s "Add to Cash Flow" context-menu action has **no invoice-level selection at all** (it pushes a coarse aged-bucket amount). Nothing to lock there — left unchanged, by design.
...
- **`APSummaryTable.jsx`'s "Make Payment" entry point still can't lock rows** on "Add to Cash Flow" — its data source (`get_ap_summary_data`'s 4-arg overload) has no `lines`/`id` array at all for its conceptual invoices, only aggregate amounts. This matches today's existing lump-sum behavior exactly (not a regression), but means the locking improvement only reaches `SupplierTx.jsx`'s "Make Payment" and `ReconcileSupplier.jsx`, not this particular entry point, until/unless that RPC is separately extended.
