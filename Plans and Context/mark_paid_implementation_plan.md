...
**Live testing on test.kensauto.ca (2026-08-30) — 2 real bugs found and fixed, everything else confirmed working:**
1. **Bug found & fixed:** "Add to Cash Flow" locked the rows correctly (verified via SQL) but the ledger page never refreshed to show it — `AddToSheetModal`'s `onSuccess` only closed the modal (`handleClose`/`onClose`), it never called the parent's `onPaymentMade`/`onPaymentComplete` refresh. A manual page reload showed the "Pending Payment" badge correctly, proving the badge logic itself was right — only the auto-refresh was missing. Fixed in both `LineOfCreditPaymentModal.jsx` (new `handleAddedToCashFlow` calls `onPaymentMade()` then `handleClose()`) and `SupplierPaymentModal.jsx` (its `AddToSheetModal`'s `onSuccess` now calls `onPaymentComplete()` then `onClose()`). **This fix is not yet pushed** — needs another commit/push before it's live.
2. **Bug found & fixed:** `SupplierTx.jsx`'s own "Invoice Lines" ledger grid (`SupplierTxInvoiceLinesTab.jsx`) already had a complete lock-driven UI (lock icon, disabled context-menu items) keyed off `isLineLocked` — but its data comes from the `get_supplier_transactions_optimized` RPC, whose line-level JSON never included `pending_cash_flow_entry_id` in any of its three line-projections (`conceptual_invoices_raw.lines`, `v_invoice_lines_date_range`, `v_all_lines_enriched`). So a freshly-locked line showed no lock indicator on this page even after a hard reload, even though `SupplierPaymentModal`'s own supplementary lock query (unaffected, separate fetch) worked fine. Fixed by adding `sil.pending_cash_flow_entry_id` to all three projections in the RPC (deployed to dev directly + saved to `src/supabase/get_supplier_transactions_optimized.sql`, previously untracked). Also fixed a third stale "why is this locked" message missed in the original pass — the lock-icon tooltip in `SupplierTxInvoiceLinesTab.jsx` still said "payment applied or is an inventory line," not mentioning the cash-flow-sheet case.
3. **Confirmed working via direct SQL + UI walkthrough, no changes needed:** the `add_to_cash_flow_atomic` RPC locks exactly the checked rows and never steals an existing lock; a fully processed payment clears `pending_cash_flow_entry_id` and flips the badge to "Paid" (both LOC and supplier); deleting a `CashFlowEntry` row from the cash sheet grid (via its right-click "Delete Row") releases the lock automatically through the FK with no application code involved; and the multi-batch "Pay from Cash Flow Sheet" dropdown does exactly what was designed — created two separate pending entries for the same supplier (CORNERSTONE COOP, $97.29 and $166.14), reopened Make Payment, and confirmed it defaulted to the older batch with its invoices pre-checked while the other batch's invoice showed "🔒 on another batch" and was disabled.
...
### Phase 1 — Schema: lock column + atomic "add to cash flow" RPC [Tested]
...
### Phase 2 — Server-side enforcement: reject edit/delete on locked or paid rows [Tested]
...
### Phase 3 — Wire "Add to Cash Flow" to actually lock the selected rows [Tested]
...
### Phase 4 — Client-side: show the lock, disable controls, pre-select in Make Payment [Tested]
...
### Phase 5 — Payment/cancellation handoff, cash-sheet delete path, full verification [Tested]
