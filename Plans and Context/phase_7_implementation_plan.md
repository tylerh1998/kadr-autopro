...
**Parent:** `master_blueprint.md` Phase 7 · **Created 2026-08-18** · **Status: Approved — ready to execute** (both open questions resolved 2026-08-18, see §0.1)
...
- [ ] `/paypro/Remittances` lists only paid, unremitted, non-cancelled paystubs — a stub already in a `status: 'completed'` remittance does **not** appear
- [ ] Selecting stubs shows correct running totals (gross, income tax, CPP emp/empr, EI emp/empr using the *current* `TaxYearConstant.ei_rate_employer_multiplier`, total remittance)
- [ ] Generate Preview → preview text matches the totals shown on-screen
- [ ] Confirm & Process Remittance → `PayPro_Remittance` row created with correct totals/`pay_stub_ids`/`status: 'completed'`; exactly 4 `GLTransaction` rows + 1 `BankTransaction` row inserted, `SUM(debit) = SUM(credit)` across the GL batch; bank account balance moves by exactly `total_remittance`; report PDF opens automatically in a new tab with matching figures
- [ ] Those same stubs now show as "Remitted"/locked on `/paypro/PayStubs` and no longer appear as available on `/paypro/Remittances`
- [ ] A remittance date inside a closed `FiscalPeriod` is rejected **before any write** — confirm via a subsequent query that zero rows were inserted (`PayPro_Remittance`, `GLTransaction`, `BankTransaction` all unchanged), not just that an error appeared
- [ ] A remittance date with no covering `FiscalPeriod` at all is also rejected, with a distinct, correct message
- [ ] `RemittanceHistory` lists all remittances (old + new), correct status badges, "Report"/"Download Text" both produce figures matching the stored row
- [ ] **Cancel Payment on one of the 5 pre-existing legacy remittances correctly fails** with the "no original posting found" message, zero rows written (D6 — expected behavior, confirm it reads that way to a user, not as a generic crash)
- [ ] Cancel Payment on a remittance created via this phase's own flow → exact-inverse `GLTransaction` set posted (4 rows), one reversing `BankTransaction`, bank balance returns to its pre-remittance figure, `status: 'cancelled'`, original rows still present and traceable
- [ ] After cancelling, that remittance's stubs reappear as available on `/paypro/Remittances` (D2/O-7) and no longer show "Remitted" on `/paypro/PayStubs`
- [ ] A cancellation date inside a closed `FiscalPeriod` is rejected before any write, same as Mark Paid
- [ ] `Remittances.jsx`'s "Manage Period Close Date" pointer button correctly navigates to `paypro/Setup`
- [ ] Both light and dark mode: no unstyled elements in `Remittances.jsx`, `RemittanceDialog`, `RemittanceHistory`, `CancelRemittanceModal` (the report PDF window is exempt — print document, light-only by design per D3)
- [ ] `grep -r "base44"` / `"@/entities/all"` in every new/modified file: zero matches (comments referencing base44 for context are fine, per Phase 3/6 precedent)
- [ ] `git status` confirms no PayPRO source file was copied verbatim without its required change
...
|---|---|---|
| 2026-08-18 | 2026-08-18 (build) | All 6 files written (5 new: `RemittanceDialog.jsx`, `CancelRemittanceModal.jsx`, `RemittanceHistory.jsx`, `RemittanceReportPDF.jsx`, `Remittances.jsx` page rewrite; 1 modified: Phase 6's `PayStubs.jsx` D2 filter). No new edge functions (Q1). `ClosePeriodModal.jsx` not ported (Q2) — replaced with a "Manage Period Close Date" button on `Remittances.jsx` navigating to `paypro/Setup`. `npx vite build` clean, zero live base44 references. Not yet live-browser-verified. |
