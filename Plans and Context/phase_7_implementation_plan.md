...
**One real, confirmed bug found — not a false alarm:** `RemittanceDialog.jsx`'s `handleSubmit` (lines ~202-291) does all its writes (create the `Remittance` row, insert the `BankTransaction`, insert the 4 `GLTransaction` rows, recalculate the bank balance, `alert()` a success message) **before** attempting to auto-open the remittance report in a new window (`window.open("", "_blank")` → `pdfWindow.document.write(pdfHTML)`). If `window.open()` returns `null` — which happens whenever the browser blocks the popup, confirmed to happen in this testing environment and a real, non-hypothetical risk for real users with a popup blocker enabled — `pdfWindow.document.write(...)` throws `TypeError: Cannot read properties of null (reading 'document')`. This is caught by the `catch` block, which calls `setError(...)` and leaves the dialog open showing an error message, **even though the underlying remittance was already fully and correctly posted** (confirmed via SQL both times this was hit during testing). Critically, `onComplete()` (line 291, which presumably closes the dialog and clears selection state) is never reached, and `finally { setProcessing(false) }` **re-enables the "Confirm & Process Remittance" button** — so a user confused by the erroneous-looking failure could click it again, which would re-run the entire `handleSubmit` against the same still-selected stubs and **create a second, duplicate remittance with duplicate GL/Bank postings for the same paycheques**. Not exercised as an actual duplicate submission during this pass (deliberately avoided, to not leave a real double-posting artifact) — the risk is established directly from the code path and the two confirmed real occurrences of the underlying `window.open()` failure. **Recommended fix (not applied — flagging only, per this agent's live-verification mandate):** move the success `alert()`/`onComplete()` call before the report-opening step (or wrap the report-opening step in its own try/catch that doesn't affect the outer success path), so a popup-blocked report doesn't get confused with a failed remittance. Phase 6's `BatchPaymentModal.jsx`/`CancelPaymentModal.jsx` don't have this exact issue (Phase 6's PDF generation is server-side, triggered separately from the payment-processing dialogs, not chained into the same try block) — this is a Phase-7-specific pattern, introduced because `RemittanceReportPDF.jsx` was ported byte-for-byte including its auto-open-report step (D3).
