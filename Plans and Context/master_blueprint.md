...
**Status:** APPROVED — Phase 1, 2, 3, 4 & 5 Tested and complete; Phase 6 up next
...
| 5 | Customer, Vehicle & GL Cleanup (transport-layer only) | [In Progress] | 2 | No schema risk |
| 6 | Reports Module | [In Progress] | 1–2 | Isolated, read-only |
| 7 | Inventory Completion (incl. Category/Location/Return hybrid finish) | [Researching] | 3–4 | |
...
| 12 | Appointment Completion | [Pending] | 2–3 | Scoped against the existing `appointment_implementation_plan.md` |
...
### Phase 5 — Customer, Vehicle & GL Transport-Layer Cleanup [Tested]

**TL;DR:** Replaced all 52 call sites across 21 files with direct `supabase.from()`/`supabase.rpc()` calls or one of 4 new native `autopro-*` Edge Functions. Data was already fully native — pure transport-layer cleanup, no schema risk, no writes lost.

**Full plan:** `Plans and Context/phase_5_implementation_plan.md` (source of truth — full 48-row call-site inventory across 21 files, verified against actual source reads of all 11 base44 functions being replaced, live schema/RLS/audit-field checks, 4 resolved scope-boundary questions, and updated in place with actual execution outcomes).

**Impacted files/functions (verified count, supersedes the original "~34 call sites" estimate — actual is 52 call sites across 21 files):** `AppointmentForm.jsx`, `CustomerForm.jsx`, `DocumentEditor.jsx`, `Schedule.jsx`, `EditApptViaWoModal.jsx`, `CashDrawer.jsx`, `CustomerARTransactions.jsx`, `CustomerARSummary.jsx`, `ARPaymentDetailsModal.jsx`, `Customers.jsx`, `Vehicles.jsx`, `NewVehicleModal.jsx`, `VehicleForm.jsx`, `DepositDetailsModal.jsx`, `InvoicePaymentModal.jsx`, `ChangeCustomerModal.jsx`, `MergeCustomerModal.jsx`, `MergeVehicleModal.jsx`, `NewWorkOrderModal.jsx`, `AddLegacyInvoiceModal.jsx`, `WorkOrderProfitability.jsx`. Base44 functions replaced: `supabaseCustomer`, `supabaseVehicle`, `supabaseCustomerPayments`, `supabaseCustomerARAdjustment`, `supabaseCustomerARSummary`, `searchCustomers`, `searchVehicles`, `mergeCustomers`, `mergeVehicles`, `decodeVin`, plus `SupabaseProxy` calls against `Customer`/`Vehicle`/`GLTransaction` within these 21 files specifically. New: `supabase/functions/autopro-mergeCustomers/`, `autopro-mergeVehicles/`, `autopro-decodeVin/`, `autopro-supabaseCustomerARSummary/`, deployed to both dev and production.
...
**Verification outcome:** Live-tested against the dev branch via the same `tyler@kensauto.ca` dev-login mechanism Phase 4 established. `Customer` create/update and both search paths (RPC + plain paginated) were live UI-tested; all 4 new Edge Functions were verified by direct invocation (seeded duplicate test records for both merges, a real VIN for decode, seeded payments/adjustments for the AR summary); the `CustomerPayments`/`GLTransaction` insert patterns underlying `DocumentEditor.jsx`/`CashDrawer.jsx` were verified directly against the authenticated dev-login session. The remaining files reuse these exact same proven patterns and were verified by code review + a clean build rather than individual UI click-throughs — logged honestly in the plan doc's checklist rather than claimed as fully tested.

**Three real bugs found and fixed during this verification pass:**
1. `autopro-mergeVehicles` imported `@base44/sdk` via `https://esm.sh/...`, which fails at Deno Edge Function worker startup with a bare `WORKER_ERROR` — switched to Deno's native `npm:@base44/sdk@0.8.24` specifier (matching how the original Base44-hosted functions imported it).
2. `autopro-mergeVehicles` called `createClientFromRequest(req)` at the top of the function, before any Vehicle/WorkOrder merge logic ran. This throws synchronously (`"Base44-App-Id header is required..."`) whenever the function is invoked via `supabase.functions.invoke()` — which has no way to attach Base44-SDK-specific headers — so it was failing the *entire* merge, not just the (already-known-to-be-skippable) Appointment-reassignment step. Fixed by moving the call inside the Appointment-specific try/catch, so the native Vehicle/WorkOrder merge always completes regardless of Base44 auth availability. Confirmed via direct invocation after the fix — mileage/odometer "keep newest" logic, field-fill, and deactivation all landed correctly.
3. All 4 new Edge Functions returned non-200 HTTP statuses (400/404/500) on error paths, violating `master_context.md` Section 3's documented convention ("Edge Functions must return `200 OK` with `{ error }` in the body — the Supabase JS client swallows the response body on non-2xx codes"). Caught via a cross-check against `master_context.md` during this same investigation (see the now-resolved note this replaces). Fixed by normalizing every non-200 status in all 4 functions to `200`; the frontend call sites already defensively checked `response.data?.error` in the body (not just the top-level `error` from `supabase.functions.invoke()`), so no frontend changes were needed — only the backend status codes. Redeployed to both environments and reconfirmed (`curl` with a bad VIN now returns `HTTP_STATUS:200` with the error message in the body).
...
**Carried over from present investigation (2026-08-03):** `autopro-processInventoryReceipt`, `autopro-processPartsInvoiceOCR`, `autopro-splitInvoicePDF` (all called from `InventoryAdd.jsx`/`EditInventoryTransactionModal.jsx`/`InventoryTransactionsModal.jsx`/`PartsInvoiceOCRModal.jsx`), and `autopro-processQOHAdjustment` (called from `InventoryAdjustQOHModal.jsx`) all currently return non-200 statuses on error, violating this doc's Edge Function convention (Section 3). Fix alongside this phase's other inventory work.
...
**Impacted files:** `components/work-orders/DocumentEditor.jsx` (8+ distinct Base44 function calls — the single biggest offender in the app), `hooks/useDocumentEditorSave.jsx`, `FindPartModal.jsx`, `GetPartModal.jsx` (shared with Phase 7 — see below), `ReceivePartModal.jsx`, `ReturnWOPartModal.jsx`, `WorkOrderPdfModal.jsx`, `ROApprovalsModal.jsx`, `SESEmailModal.jsx`, `pages/WorkOrders.jsx`, `InvoiceConversion.jsx`, `CreditInvoice.jsx`.
...
**Carried over from present investigation (2026-08-03):** Two Edge Function violations of this doc's `200`-status convention (Section 3) were found and fixed as a minimal drive-by (not deferred, since they were already live and their bug — a `TypeError` on the frontend instead of a real error message — could hit production at any time): `autopro-archiveWorkOrderProjects` (built in Phase 4, already Tested) and `autopro-WOBulkGetParts` (pre-blueprint baseline, called from `GetPartModal.jsx`), plus `GetPartModal.jsx`'s response handling. Since both are core work-order-lifecycle paths (Get Part, and archiving projects on invoice conversion), this phase's full WO-lifecycle regression pass should still explicitly re-verify them rather than treat them as closed out. Still outstanding for this phase to fix (found during the same pass, not yet touched): `autopro-processWorkOrderPartReceive` (`ReceivePartModal.jsx`) and `autopro-processWorkOrderPartReturn` (`ReturnWOPartModal.jsx`) both return non-200 on error; `autopro-processDeletedWorkOrderLineItem` and `autopro-saveworkorderdata` (both via `useDocumentEditorSave.jsx`, already listed above) need the same fix. **Explicitly excluded:** `autopro-handleInvoiceConversionGL`/`autopro-handleSupplierInvoiceLineGL` are not part of this cleanup — Risk Assessment #2 above says never modify the already-native GL posting functions, full stop, even for this convention fix.
...
- **2026-08-03 (found during Phase 6 planning, distributed across phases):** A post-approval cross-check of Phase 6's new functions against this doc's `200`-status Edge Function convention (Section 3) turned up ~16 other `autopro-*` functions already violating it, including two already-live ones with a real (if rare) frontend crash risk (`autopro-archiveWorkOrderProjects`, `autopro-WOBulkGetParts` — both fixed immediately as a drive-by, see Phase 4/13 entries above). Rather than sweep the rest in one pass, each remaining violation was logged directly in its actual owning phase's "Impacted files"/carry-over notes (Phase 5: merge/VIN/AR-summary functions; Phase 7: inventory-receipt/OCR/split-PDF/QOH-adjustment; Phase 13: WO part-receive/return) instead of bundled into whichever phase happened to be active — two of the candidate functions (`autopro-handleInvoiceConversionGL`/`autopro-handleSupplierInvoiceLineGL`) are explicitly excluded since Risk #2 already forbids touching them. **General rule for future phases:** when a documented cross-cutting convention gets violated somewhere unrelated to the phase you're planning, don't fix everything at once just because it's fresh in context — file it against the phase that actually owns that code, the same way Phase 6's payroll-progress-bar item was carried to Phase 10.
- **2026-08-03 (Phase 5 verification, closes the loop on the entry above):** Phase 5's carried-over `200`-status violation (`autopro-mergeCustomers`/`mergeVehicles`/`decodeVin`/`supabaseCustomerARSummary`) is now fixed and reconfirmed — all four normalized to always return `200` with `{ error }` in the body, redeployed to both environments, spot-checked with a deliberately-bad VIN (`curl` confirms `HTTP_STATUS:200`). No frontend changes were needed to consume the fix: the call sites already defensively read `response.data?.error` rather than relying solely on `supabase.functions.invoke()`'s top-level `error` (which only fires on network failure or non-2xx, and non-2xx is exactly the case this convention eliminates). **Worth carrying forward as a frontend-call-site pattern**, not just a backend rule: any new `autopro-*` invocation should check the body's `error` field regardless of whether the top-level `error` came back, since a future backend regression to non-2xx would otherwise silently degrade to a generic swallowed message.
- **2026-08-03 (Phase 5 verification):** **`npm:` vs `https://esm.sh/` matters for Deno Edge Function imports of `@base44/sdk` specifically — they are not interchangeable.** `autopro-mergeVehicles` imported it via `https://esm.sh/@base44/sdk@0.8.24` (matching this repo's own convention for `@supabase/supabase-js`) and the function crashed at worker startup with a bare `WORKER_ERROR` on every invocation, no diagnostic detail. Switching to Deno's native `npm:@base44/sdk@0.8.24` specifier — the exact form every original Base44-hosted function in `base44/functions/*/entry.ts` already uses — fixed it immediately. Rule for any future phase porting a Base44-hosted function that imports `@base44/sdk`: copy the original's `npm:` import verbatim, don't normalize it to this repo's usual `esm.sh` pattern for other packages.
- **2026-08-03 (Phase 5 verification, structural finding, relevant to any future `@base44/sdk` usage inside an `autopro-*` function):** **`createClientFromRequest(req)` from `@base44/sdk` throws synchronously if the request lacks Base44-SDK-specific headers (`Base44-App-Id`, etc.) — and a request sent via `supabase.functions.invoke()` from the migrated frontend can never carry those headers**, since that call goes through the Supabase JS client, not the Base44 SDK. Any native `autopro-*` function that still needs to reach into Base44 (e.g. `autopro-mergeVehicles`'s `Appointment` reassignment, pending Phase 12) must call `createClientFromRequest(req)` *lazily, inside its own try/catch, and only immediately before the Base44-specific call* — never at the top of the function — or the entire function fails before any of its native Postgres logic runs, even when that native logic doesn't depend on Base44 at all. In practice this means such a block will **always** hit its catch branch when invoked from the frontend today; that's an accepted no-op, not a bug, until the dependency itself (Appointment, in this case) migrates off Base44.
