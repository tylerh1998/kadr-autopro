...
- **P12** — **New, found 2026-08-13 during live 15C testing, unrelated to base44/Phase 15.** `WorkOrder.line_items` is a genuine jsonb array column, but a large historical share of rows hold it as a JSON *string* instead of a true array (`jsonb_typeof = 'string'`) — historical, not actively growing (the `autopro-saveworkorderdata` Edge Function already unwraps a string payload defensively before writing; the frontend double-encoding root cause in `buildWorkOrderSavePayload.js` is also fixed). The resulting crash in "Find Part in Work Orders" (`search_work_order_parts` RPC) is fixed and **deployed to both dev and production**. **Backfilling the already-corrupted rows themselves: plan drafted 2026-08-14, execution not yet started.** Full 3-stage plan (controlled single-record → whole dev DB → production, each gated on the previous stage's validation): `p12_line_items_backfill_plan.md`. Includes a newly-surfaced finding during planning: `WorkOrder` has a second trigger (`audit_workorder_changes`) that would otherwise log a spurious "material change" history entry for every backfilled row — the plan disables it for the duration of each stage's `UPDATE`. Sample ROs for reference: **RO51610** (corrupt, the Stage 1 canary), **RO51619** (clean) — both on dev.
