-- Production counterpart of 20260815232828_rewrite_set_workorder_lock_remove_auto_steal.sql
-- (Work Order Locking Remediation, Plans and Context/implementation_plan_wo_locking.md,
-- Phase 6). Same content, applied separately to hbcrwkmgsazqrvsrmxyr (production)
-- via apply_migration, which assigned its own migration version distinct from
-- dev's -- see master_context.md's note on the two Supabase projects having
-- independent migration histories that can diverge even for the same logical
-- change. Confirmed byte-identical to dev's live definition via
-- pg_get_functiondef on both projects before this file was written.
--
-- Behavior changes from the pre-remediation version:
