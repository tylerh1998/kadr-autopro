-- Production counterpart of 20260816034659_add_staff_strong_auth_rls.sql,
...
-- on the two Supabase projects having independent migration histories. The
-- migration is written idempotently (drop-if-exists before every create)
-- specifically so the same content is safe to apply to both projects
-- regardless of their differing starting policy shapes -- see the dev file
-- for the full design rationale and Plans and Context/rls_strong_auth_policy_plan.md
-- for verification evidence on both projects.
