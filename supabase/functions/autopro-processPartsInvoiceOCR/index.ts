...
serve(async (req) => {
    ...
    try {
        let { pdfData, storagePath, mimeType = 'application/pdf', supplierNames = [], documentContext = 'invoice' } = await req.json();
        ...
        let documentContextNote = '';
        if (documentContext === 'wo_parts') {
