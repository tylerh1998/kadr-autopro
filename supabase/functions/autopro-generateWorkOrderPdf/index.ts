...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: 'Invalid JSON payload', details: jsonError.message }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    if (!workOrder) {
      return new Response(JSON.stringify({ error: 'Work Order data missing' }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    const drawFinancialBar = (doc) => {
      ...
      if (workOrder?.ro_number) {
        ...
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(0);
        doc.text(String(workOrder.ro_number), margin, barY - 4);
      }
    ...
    // Process line items (Core logic) — Core_num/core_ret/core_cost/core_osamt per §0.6 correction
    ...
    // Return as base64 JSON to avoid binary handling issues with the client
